...
- "What happened on <date/day>", "when did I see/talk to <person>", "who is <logbook person>", "who have I not seen/mentioned lately", "who should I message/check in with", "where was I", "show my journal/logbook", mood/datapoint history, people/places from daily entries -> use `manage_logbook`. It is read-only diary evidence; cite dates and do not present suggested person connections as facts unless accepted.
...
Notes, checklists, AND user reminders. Use this for "create/add/write a note", todos, checklists, and "remind me to X at <time>" — never use memory for note content. For reminders, pair a short `title` (what to do) with a `due_date` (when). `due_date` accepts natural language ("tomorrow at 1pm", "in 2 hours", "next monday 9am") or ISO ("2026-05-12T13:00:00"). Actions: `list`, `add` (title, content OR items:[{text,done}], note_type, color, label, due_date), `update`, `delete`, `toggle_item`.""",
    "manage_logbook": """\
```manage_logbook
{"action": "get_day", "date": "today"}
```
Read-only Daily Logbook recall. Use for journal/logbook/diary questions, "what happened on <date/day>", "when did I see/talk to <person>", "who is <person>", "who have I not seen lately", "who should I message/check in with", places/locations, mood, sleep/energy/stress/workout/gratitude datapoints, timelines, person/place context, reconnect hints, and accepted/suggested person connections. Actions: `get_day`, `list_range`, `timeline`, `search`, `people`, `places`, `directories`, `person_detail`, `place_detail`, `connections`. Filters: date/start/end/q/person/place/name/mood/datapoint_key/status/limit. Cite dates. Treat suggested connections cautiously; only accepted connections are user-confirmed.""",
